...
def run_financial_model(inputs, project_type):
    ...
    df['pv_ppa_income'] = (inputs.get('pv_income_ppa_y1',0) * df['pv_degradation_factor'] * df['inflation_factor']) if is_pv else 0
...
def create_kpi_dataframe(kpi_dict, section_title, unit_map):
    data = []
    for key, value in kpi_dict.items():
        name = key.replace(f"{section_title}_", "").replace("_", " ").title()
        unit = unit_map.get(key, " ")
        if unit == "€": formatted_value = f"€ {value:,.0f}"
        elif unit == "%": formatted_value = f"{value:.2%}"
        elif unit == "h": formatted_value = f"{value:.2f}"
        else: formatted_value = f"{value:,.0f}"
        data.append({"Metric": name, "Value": formatted_value, "Unit": unit})
...
if uploaded_file:
    ...
    st.sidebar.success("CSV Uploaded (Full parsing logic to be fully implemented)")
...
if 'BESS' in st.session_state.project_type:
    ...
    with st.sidebar.expander("Technical Inputs (BESS)", expanded=False):
        ...
        i['bess_capacity_kwh'] = st.number_input("Storage Capacity (kWh)", value=i['bess_capacity_kwh'], help="Total nominal energy capacity.", key='bess_c_kwh')
