...
class BatteryShavingAnalyzer:
    ...
    def run_analysis(self, input_df: pd.DataFrame):
        ...
        # 3. Calculate Energy and State of Charge (SOC) - CORRECTED VERSION
        df["energy_through_battery"] = df["battery_power"] * self.time_step_h
        
        # PROPER SOC CALCULATION: Track SOC iteratively with realistic constraints
        soc_values = []
        current_soc = 0.0
        
        # First, find the minimum SOC needed to avoid going negative
        # This ensures the battery never has negative energy
        cumulative_energy = 0.0
        min_cumulative = 0.0
        
        for energy in df["energy_through_battery"]:
            cumulative_energy += energy
            min_cumulative = min(min_cumulative, cumulative_energy)
        
        # Start with enough energy to cover the deepest discharge
        initial_soc = -min_cumulative if min_cumulative < 0 else 0.0
        current_soc = initial_soc
        
        # Now calculate SOC properly
        for energy in df["energy_through_battery"]:
            current_soc += energy
        ...
        # 4. Determine required size from the analysis
        if not df["soc_kwh"].empty:
            # This is now correct: the range between min and max SOC is the battery capacity needed
            required_capacity_kwh = df["soc_kwh"].max() - df["soc_kwh"].min()
        else:
            required_capacity_kwh = 0.0
